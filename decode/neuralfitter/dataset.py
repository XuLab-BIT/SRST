...
class SMLMDataset(Dataset):
    ...
    def _process_sample(self, frames, tar_emitter, bg_frame, nobg_frames):

        """Process"""
        if self.frame_proc is not None:
            frames = self.frame_proc.forward(frames)
            # nobg_frames = self.frame_proc.forward(frames)
        ...
        return frames, target, weight, tar_emitter, nobg_frames

    def _return_sample(self, frame, target, weight, emitter, nobg_frame):

        if self.return_em:
            return frame, target, weight, emitter, nobg_frame
        else:
            return frame, target, weight, nobg_frame
...
class SMLMStaticDataset(SMLMDataset):
    ...
    def __init__(self, *, frames, emitter: (None, list, tuple),
                 ...
                 bg_frames=None, weight_gen=None, frame_window=3, pad: (str, None) = None, return_em=True, nobg_frames=None):
        """
    ...
    def __getitem__(self, ix):
        ...
        nobg_frame = self._nobg_frames[ix] if self._nobg_frames is not None else None

        frames, target, weight, tar_emitter, nobg_frame = self._process_sample(frames, tar_emitter, bg_frame, nobg_frame)

        return self._return_sample(frames, target, weight, tar_emitter, nobg_frame)
...
class SMLMLiveDataset(SMLMStaticDataset):
    ...
    def sample(self, verbose: bool = False):
        ...
        """Sample new dataset."""
        t0 = time.time()
        emitter, frames, bg_frames, nobg_frames = self.simulator.sample()
        if verbose:
        ...
        self._nobg_frames = nobg_frames.cpu()
...
class SMLMAPrioriDataset(SMLMLiveDataset):
    ...
    def sample(self, verbose: bool = False):
        ...
        frames, target, weight, tar_emitter, nobg_frames = self._process_sample(frames, emitter, bg_frames, nobg_frames)
        self._frames = frames.cpu()
...
class SMLMLiveSampleDataset(SMLMDataset):
    ...
    def __getitem__(self, ix):
        ...
        nobg_frames = nobg_frames[(self.frame_window - 1) // 2]

        frames, target, weight, tar_emitter, nobg_frame = self._process_sample(frames, tar_emitter, bg_frames, nobg_frame)
