...
class EmitterSamplerBlinking(EmitterSamplerFrameIndependent):
    def __init__(self, *, structure: structure_prior.StructurePrior, intensity_mu_sig: tuple, lifetime: float,
        ...
        self.n_blink = np.random.poisson
    ...
    def sample_loose_emitter(self, n) -> decode.generic.emitter.LooseEmitterSet:
        ...
        blink_time = self.n_blink(1,n)
        return generic.emitter.LooseEmitterSet(xyz, t0, id=torch.arange(n).long(),
                                                      xy_unit=self.xy_unit, px_size=self.px_size, blink_time=blink_time, 
                                                      lifetime_dist=self.lifetime_dist, intensity_dist=self.intensity_dist, intensity_th=self.intensity_th)
    
    def dist_emitter(self, xyz) -> decode.generic.emitter.EmitterSet:

        n = xyz.size()[0]
        t0 = self.t0_dist.sample((n,))
        blink_time = self.n_blink(1,n)
        
        loose_em = generic.emitter.LooseEmitterSet(xyz, t0, id=torch.arange(n).long(),
                                                      xy_unit=self.xy_unit, px_size=self.px_size, blink_time=blink_time, 
                                                      lifetime_dist=self.lifetime_dist, intensity_dist=self.intensity_dist, intensity_th=self.intensity_th)
        em = loose_em.return_emitterset()
        em = em.get_subset_frame(*self.frame_range)
